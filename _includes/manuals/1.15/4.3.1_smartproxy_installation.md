...
1. In order to use the service as a domain account, the domain user needs to be added to the local admin group, as well as enallow that user rights to "Log on as Service".  You may run into "Error 1053: "The service did not respond in a timely fashion", if these are not set correctly.
